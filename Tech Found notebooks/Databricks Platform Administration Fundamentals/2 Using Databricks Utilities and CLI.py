...
display(dbutils.fs.ls("/tmp/token"))

# COMMAND ----------

# MAGIC %fs rm dbfs:/user/tmp/token
...
# MAGIC %md
# MAGIC create token folder inside catalog first

# COMMAND ----------

dbutils.fs.put('/Volumes/asvi_test/default/token/token', dbutils.widgets.get('token'), True)

# COMMAND ----------

# MAGIC %sh databricks configure --host https://one-env-asvi-test.cloud.databricks.com/ --token-file /Volumes/asvi_test/default/token/token
...
# MAGIC %sh databricks configure --host https://e2-demo-field-eng.cloud.databricks.com/ --token-file /dbfs/tmp/token

# COMMAND ----------

# MAGIC %sh databricks tokens list
