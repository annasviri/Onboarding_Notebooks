...
-- MAGIC
-- MAGIC A sample dataset of flights data is created for you. **The table is partitioned by `origin` field and Change Data Feed(CDF) is specified which will allow us to share partial data and share data with CDF enabled.**
-- MAGIC
...
-- MAGIC
-- MAGIC A share is a read-only collection of tables and table partitions to be shared with one or more recipients. Let's first create a share and then specify the partition details while adding a table to the share.
-- MAGIC
-- MAGIC **📌 Important:** Before sharing a table with partition specification, make sure the table is partitioned by the column. In this demo, the `flights` table created in the setup script is already **partitioned by `origin` column** when it is created. This means we can share partial data of this table by specifying `origin` column. *However, if we try to share a subset of dataset by `destination` column we will get an error as the table is not paritioned by this column.*   
-- MAGIC
